...
1. **Clone the repo**
   ...
   ```bash
   git clone <your-repo-url> ai-vis-standards
   cd ai-vis-standards
   ```
...
> * Practical advice: run it **after each batch** (e.g., you finish a model × condition across a task), and at **end of day**.
...
* **Disable browsing/tools** in the web UI if possible. If the UI auto-retrieves (e.g., shows citations), note it—add a short comment to `stderr.txt` or a `notes` field in `run.json` if you extend the schema.
